...
export async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    `);
    await db.query(`ALTER TABLE organizations ADD COLUMN IF NOT EXISTS email TEXT`);
    await db.query(`ALTER TABLE organizations ADD COLUMN IF NOT EXISTS plan TEXT NOT NULL DEFAULT 'free'`);
    await db.query(`ALTER TABLE organizations ADD COLUMN IF NOT EXISTS stripe_customer_id TEXT`);
    await db.query(`ALTER TABLE organizations ADD COLUMN IF NOT EXISTS stripe_subscription_id TEXT`);
    await db.query(`ALTER TABLE organizations ADD COLUMN IF NOT EXISTS pages_used INT NOT NULL DEFAULT 0`);
    await db.query(`ALTER TABLE organizations ADD COLUMN IF NOT EXISTS pages_quota INT NOT NULL DEFAULT 10000`);
    await db.query(`ALTER TABLE organizations ADD COLUMN IF NOT EXISTS period_reset_at TIMESTAMPTZ NOT NULL DEFAULT NOW()`);
    await db.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS organizations_email_lower_idx
      ON organizations (lower(email)) WHERE email IS NOT NULL
    `);
